    ...
    try {
      ...
      if (!res.ok) {
        ...
        const messages: Record<string, string> = {
          ...
          self_invite: "That's your own email — enter your parent's instead.",
          already_linked: 'A parent is already linked to your account.',
          send_failed: "Couldn't send the invite email — try again.",
        };
        toast.error(messages[(data as { error?: string })?.error ?? ''] ?? 'Something went wrong — try again.');
        return;
      }
      toast.success('Invite sent! Ask your parent to check their email.');
      setSent(true);
      setParentEmail('');
    } catch {
        ...
        <section className="rounded-lg border border-border p-4">
          ...
          {currentUser.parentId ? (
            ...
            <>
              ...
              {sent ? (
                <p className="mt-3 text-sm text-foreground">Invite sent — check back once they&apos;ve signed up.</p>
              ) : (
                <div className="mt-3 flex gap-2">
                  ...
                  />
                  <Button size="sm" disabled={sending} onClick={() => void handleInvite()}>
                    {sending ? 'Sending…' : 'Add a parent'}
                  </Button>
                </div>
              )}
            </>
